...
3.  Home Page - shows answered and unanswered questions
4.  Poll Detail Page - links from polling question
5.  Add New Poll Question Page
6.  Leaderboard Page
7.  404 Page when accessing an unauthorized page
...
Cleaner UI, Registration, Redux Persist

scaling (how would you handle displaying thousands of polls/questions under hundreds of categories)
error handling (Should it be done in Redux actions/reducers? Should it be done in the component?)
testing (which testing libraries would you use to test React/Redux?)
how will user experience be improved (you could convert it into a PWA (Progressive Web Application),
how will content be cached to improve loading speeds for users (hint: research 'LRU caching')?
which algorithm is best for quickly sorting a large API response (potentially up to hundreds) from the server,
